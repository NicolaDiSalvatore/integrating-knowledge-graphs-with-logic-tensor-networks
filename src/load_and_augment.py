...
def vectorized_extract_keywords(synsets):
    ...
    :param synsets (list of str): A list of synset strings, where each synset follows the format 'lemma.pos.index' (e.g., 'dog.n.01').
    :return (list of str): A list of keywords (lemmas) extracted from the input synsets, with each keyword converted to lowercase.
    """
...
# Format a concept in ConceptNet format
...
# aligned_attributes = find_similar_concepts_in_dataframe(attributes_data, embeddings_dict, vectors)
...
# objects_concepts = aligned_objects[['synsets', 'most_similar_concept', 'similarity']]
# objects_concepts['most_similar_concept'] = objects_concepts['most_similar_concept'].apply(format_concept)
# objects_concepts = objects_concepts.dropna(subset=['most_similar_concept'])
# objects_concepts['synsets'] = objects_concepts['synsets'].apply(tuple)
# objects_concepts = objects_concepts.drop_duplicates()
# objects_concepts['synsets'] = objects_concepts['synsets'].apply(list)
# objects_concepts.columns = ['synsets', 'concept', 'similarity']
# objects_concepts = objects_concepts.reset_index()
# objects_concepts = objects_concepts.drop('index', axis=1)
# joblib.dump(objects_concepts, 'unique_objects_alignment.joblib')

# The following code save the predicates alignment dropping all feature apart from concepts and the similarity

# predicates_concepts = aligned_relationships[['synsets', 'most_similar_concept', 'similarity']]
# predicates_concepts['most_similar_concept'] = predicates_concepts['most_similar_concept'].apply(format_concept)
# predicates_concepts = predicates_concepts.dropna(subset=['most_similar_concept'])
# predicates_concepts['synsets'] = predicates_concepts['synsets'].apply(tuple)
# predicates_concepts = predicates_concepts.drop_duplicates()
# predicates_concepts['synsets'] = predicates_concepts['synsets'].apply(list)
# predicates_concepts.columns = ['synsets', 'concept', 'similarity']
# predicates_concepts = predicates_concepts.reset_index()
# predicates_concepts = predicates_concepts.drop('index', axis=1)
# joblib.dump(predicates_concepts, 'unique_predicates_alignment.joblib')

# The following code save the attributes alignment dropping all feature apart from concepts and the similarity

# attributes_concepts = aligned_attributes[['synsets', 'most_similar_concept', 'similarity']]
# attributes_concepts['most_similar_concept'] = attributes_concepts['most_similar_concept'].apply(format_concept)
# attributes_concepts = attributes_concepts.dropna(subset=['most_similar_concept'])
# attributes_concepts['synsets'] = attributes_concepts['synsets'].apply(tuple)
# attributes_concepts = attributes_concepts.drop_duplicates()
# attributes_concepts['synsets'] = attributes_concepts['synsets'].apply(list)
# attributes_concepts.columns = ['synsets', 'concept', 'similarity']
# attributes_concepts = attributes_concepts.reset_index()
# attributes_concepts = attributes_concepts.drop('index', axis=1)
# joblib.dump(attributes_concepts, 'unique_attributes_alignment.joblib')


# Load the preceding stored daaframes
aligned_unique_objects = joblib.load('unique_objects_alignment.joblib')
aligned_unique_predicates = joblib.load('unique_predicates_alignment.joblib')
aligned_unique_attributes = joblib.load('unique_attributes_alignment.joblib')
